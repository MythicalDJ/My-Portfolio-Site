...
class TrailingStopLossPaperTrading:
	...
	def Buy_with_trailing_stop_loss_and_profit(self, ctx):
		...
		if self.can_day_trade(current_date):
			if ctx.long_pos():
				...
				# Store the current stop price
				self.stop_prices[ctx.symbol] = self.stop
				...
				self.current_trades[ctx.symbol] = {
					'entry_date': current_date,
					'entry_price': ctx.open[-1],
					'shares': self.shares_bought
				}

				# Update trade info
				self.trade_info.update_trade_dates(current_date)
				self.trade_info.update_position(ctx.symbol, self.shares_bought, ctx.open[-1])
		else:
	...
	def Set_stocks_to_test(self, StockList, TimeData):
		...
		first_run_date = self.trade_info.get_first_run_date()
		self.DateFrom = first_run_date if first_run_date else TimeData.DateFrom
		self.DateTo = TimeData.DateTo
		self.TimeFrame = TimeData.TimeFrame

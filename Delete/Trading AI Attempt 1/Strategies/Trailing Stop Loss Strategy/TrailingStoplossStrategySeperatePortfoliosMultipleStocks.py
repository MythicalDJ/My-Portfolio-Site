...
class Stock:
    def __init__(self, symbol, islong):
        self.symbol = symbol
        self.islong = islong
...
StockOne = Stock('spy', True)
StockTwo = Stock('aapl', True)
StockThree = Stock('msft', True)
StockFour = Stock('amzn', True)
StockFive = Stock('vug', True)
StockSix = Stock('googl', True)
...
stocks = [StockOne, StockTwo, StockThree, StockFour, StockFive, StockSix]
...
price_data = {}
for stock in stocks:
    price_data[stock] = alpaca.query(stock.symbol, DateFrom, DateTo, TimeFrame)

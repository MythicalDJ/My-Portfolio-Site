...
class TrailingStopLossPaperTrading:
	...
	def Buy_with_trailing_stop_loss_and_profit(self, ctx):
		...
		if self.daytrades < 4:
			if ctx.long_pos():
				TrailingStopLossPaperTrading.Calculate_Stop(self, ctx)
				...
				if ctx.open[-1] < self.stop:
					...
					if ctx.low[-1] < ctx.open[-1] * 0.975:
						...
						ctx._portfolio.sell(ctx._curr_date, ctx.symbol, ctx._portfolio.long_positions[ctx.symbol].entries[0].shares, decimal.Decimal(str(ctx.open[-1] * 0.975)))
					# Else if the high is greater than the stop
						...
						if ctx.close[-1] > self.stop:
							...
							# Sell at the stop
							ctx._portfolio.sell(ctx._curr_date, ctx.symbol, ctx._portfolio.long_positions[ctx.symbol].entries[0].shares, decimal.Decimal(str(self.stop)))
					else:
						self.count += 1
						# Sell at the close price
						# ctx._portfolio.sell(ctx._curr_date, ctx.symbol, ctx._portfolio.long_positions[ctx.symbol].entries[0].shares, decimal.Decimal(str(ctx.close[-1])))
			else:
				self.shares_bought = ctx.calc_target_shares(1)
				ctx._portfolio.buy(ctx._curr_date, ctx.symbol, self.shares_bought, decimal.Decimal(str(ctx.open[-1])))
